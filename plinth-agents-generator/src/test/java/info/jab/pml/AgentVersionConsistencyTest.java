...
class AgentVersionConsistencyTest {

    @TestFactory
    @DisplayName("Every agent metadata version matches the root Maven project version")
    Stream<DynamicTest> agentVersionsMatchRootPom() throws Exception {
        String expectedVersion = readRootProjectVersion();

        return AgentIndexes.agentSources()
            .map(source -> DynamicTest.dynamicTest(source, () -> assertAgentVersion(source, expectedVersion)));
    }

    private void assertAgentVersion(String source, String expectedVersion) throws Exception {
        String resourceName = "agents/" + source;
        try (InputStream stream = requireResource(resourceName)) {
            ...
            assertThat(actualVersion)
                .withFailMessage(
                    "Agent %s has metadata version '%s', but the root pom.xml version is '%s'.",
