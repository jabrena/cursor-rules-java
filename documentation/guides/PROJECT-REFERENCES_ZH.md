# 项目参考资料

> **语言：** [English](./PROJECT-REFERENCES.md) · [Español](./PROJECT-REFERENCES_ES.md)

本页汇总 `cursor-rules-java` 的演讲、文章、参考链接、skill 门户和相关项目。

## 聚会、会议、工作坊与文章

### Codemotion / Madrid (2026/04/20 - 11:00 - 12:30)

- [Taller técnico sobre Cursor para el desarrollo con Java](https://conferences.codemotion.com/madrid/speakers/)

### W-JAX / Munich (2025/11/06 - 10:30 - 11:30)

- [Cursor AI 101 Java Enterprise](https://jax.de/generative-ai-ecosystem/cursor-ai-101-java-enterprise/)

### Devoxx BE / Antwerp (2025/10/07 - 18:20 - 18:50)

- [The power of Cursor Rules in Java Enterprise Development](https://m.devoxx.com/events/dvbe25/talks/4715/the-power-of-cursor-rules-in-java-enterprise-development)

### Madrid Jug / Madrid (2025/05/06 - 19:00)

- [MadridJUG 活动](https://www.meetup.com/es-ES/madridjug/events/307458529/)

## 博客

- [Delegating Java tasks to Supervised AI Dev Pipelines](https://www.javaadvent.com/2025/12/delegating-java-tasks-to-supervised-ai-dev-pipelines.html)
- [Cursor AI developer cloud platform](https://vibekode.it/blog/cursor-ai-developer-cloud-platform/)
- [September REST story JVM Weekly vol 146](https://www.linkedin.com/pulse/september-rest-story-jvm-weekly-vol-146-artur-skowro%C5%84ski-82lif/?trackingId=wbWPSL65TpCCbdg5ksAWjw%3D%3D)
- [Providing library documentation](https://virtuslab.com/blog/ai/providing-library-documentation/)
- [Skill of Skills](https://github.com/the911fund/skill-of-skills)
- [CSDN 文章](https://blog.csdn.net/weixin_42526249/article/details/161176209)
- [Juejin 文章](https://juejin.cn/post/7632095808490340392)

## 参考资料

### AI Agents 支撑工具

- [Cursor](https://www.cursor.com/)
- [Cursor CLI](https://cursor.com/cli)
- [Claude Code](https://www.anthropic.com/claude-code)
- [GitHub Copilot](https://github.com/features/copilot)
- [Cursor GitHub Actions](https://cursor.com/docs/cli/github-actions)
- [Claude Code GitHub Actions](https://code.claude.com/docs/en/github-actions)

### 标准

- [AGENTS.md](https://agents.md/)
- [Agent Skills](https://agentskills.io/home)
- [AAIF](https://aaif.io/)
- [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)
- [OpenSpec](https://openspec.dev/)

### Skill 门户

- [skills.sh cursor-rules-java](https://skills.sh/jabrena/plinth)
- [Agent Skills Hub plinth 页面](https://agentskillshub.top/skill/jabrena/plinth/)
- [MCP App Store Mongock skills 搜索](https://mcpapp-store.com/skills?q=Mongock+)
- [JVM Skills](https://jvmskills.com/)
- [Tarai Java 现代特性重构 skill](https://tarai.dev/en/skill/141-java-refactoring-with-modern-features/)
- [Vercel Labs skills issues](https://github.com/vercel-labs/skills/issues)
- [Tessl skill registry](https://tessl.io/registry/skills/github/jabrena/plinth)
- [ClaudSkills 作者页](https://claudskills.com/author/jabrena/)
- [Claude Marketplaces Java skills 分类](https://claudemarketplaces.com/skills/category/java)
- [Agent Skills 中文门户](https://agent-skills.cc/zh/skills/jabrena-cursor-rules-java)
- [Shyft skill 页面](https://shyft.ai/skills/cursor-rules-java)
- [LobeHub skills 搜索](https://lobehub.com/skills?q=cursor-rules-java)
- [Awesome Skills 页面](https://www.awesomeskills.dev/es/skill/jabrena-cursor-rules-java)
- [Claude Code skills 中文翻译](https://github.com/laolaoshiren/claude-code-skills-zh)
- [Awesome Cursor Rules 中文翻译](https://github.com/LessUp/awesome-cursorrules-zh)
- [learn-skills.dev 作者页](https://www.learn-skills.dev/en/for/jabrena)
- [SkillsLLM cursor-rules-java](https://skillsllm.com/skill/cursor-rules-java)
- [911 Fund cursor-rules-java 工具页面](https://skills.911fund.io/tools/jabrena-cursor-rules-java)
- [SkillsLLM cursor-rules-java 与 superpowers 对比](https://skillsllm.com/compare/cursor-rules-java-vs-superpowers)
- [Open Claw directory 规则页](https://open-claw.directory/rules)

### Java

- [OpenJDK JEP 索引](https://openjdk.org/jeps/0)
- [JBake 文档](https://jbake.org/docs/latest/)
- [Spring Cloud for microservices compared to Kubernetes](https://developers.redhat.com/blog/2016/12/09/spring-cloud-for-microservices-compared-to-kubernetes)

## 其他项目

- [PML](https://github.com/jabrena/pml)
- [cursor-rules-java](https://github.com/jabrena/plinth)
- [AI Agent Harness Monitor CLI](https://github.com/jabrena/ai-agent-harness-monitor-cli)
- [setup-cli](https://github.com/jabrena/setup-cli)
