Feature: Scenario 1 — Case 1 minimal functional notes
  ...
  # - Do NOT read companion artifacts under specs/functional-requirements/ other than README.md
  #   even if they exist on disk.
  # - Do NOT search the repository for "Greek Gods API", ADR, OpenSpec, problem1, or
  #   problem2 artifacts outside benchmarks/problem2/scenario1/.
  # - Other scenarios and examples/openspec/ are out of scope for this run even if they
  #   describe a similar product.
  # - Under benchmarks/problem2/scenario1/results/, read ONLY README.md and
  #   example.result.json (operator/metrics template).
  # - Do NOT read prior run JSON files under benchmarks/problem2/scenario1/results/.
  # - Skill discovery is allowed: skills under .agents/skills/ or skills/ may be read; the
  #   model decides which skills are relevant and the result JSON records only skills
  #   actually read or invoked.

  Background:
    Given a Case 1 benchmark run for scenario "scenario1"
    And the run case id is "case-1-readme-only"
    And results are stored under "benchmarks/problem2/scenario1/results/"
    And the Case 1 allowlist is the only authorized reading set for requirements and product behavior:
      | path |
      | benchmarks/problem2/scenario1/specs/functional-requirements/README.md |
      | benchmarks/problem2/scenario1/gherkin/scenario1.feature |
      | benchmarks/problem2/scenario1/README.md |
      | benchmarks/problem2/metrics-v2.schema.json |
      | benchmarks/problem2/metrics-v2.example.json |
    And the Case 1 results allowlist is the only authorized reading set under "benchmarks/problem2/scenario1/results/":
      | path |
      | benchmarks/problem2/scenario1/results/README.md |
      | benchmarks/problem2/scenario1/results/example.result.json |
  ...
  Scenario: Case 1 run records minimal v1 metrics as JSON
    Given "benchmarks/problem2/scenario1/specs/functional-requirements/README.md" is the sole product specification
    And only files on the Case 1 allowlist are read for requirements, design, or acceptance criteria
    And only files on the Case 1 results allowlist are read under "benchmarks/problem2/scenario1/results/"
    ...
    And no file under "benchmarks/problem2/scenario1/results/" outside the Case 1 results allowlist may be read during the run
    And the agent must not read, open, grep, or search under any path outside "benchmarks/problem2/scenario1/" except harness metrics files on the Case 1 allowlist under "benchmarks/problem2/" and Plinth tooling on the Case 1 Plinth tooling allowlist
    And companion artifacts under "benchmarks/problem2/scenario1/specs/functional-requirements/" other than "README.md" must not be read as scenario input
    And a full agile/design functional-requirements package is not provided as agent input
    And "benchmarks/problem2/scenario1/specs/technical-requirements/" is not provided
    ...
    And no "ADR-*" file outside "benchmarks/problem2/scenario1/" may be read for requirements or technology choices
    And no "openspec/changes/" design, tasks, or spec files outside "benchmarks/problem2/scenario1/" may be read for requirements
    When the agent implements the product behavior documented in "benchmarks/problem2/scenario1/specs/functional-requirements/README.md" in "benchmarks/problem2/scenario1/demo/"
    And the run completes
    Then the product happy path embedded in "benchmarks/problem2/scenario1/specs/functional-requirements/README.md" passes
    And a result JSON file exists under "benchmarks/problem2/scenario1/results/"
    ...
    And the result JSON field "solution_snapshot.demo_root" equals "benchmarks/problem2/scenario1/demo/"
    ...
    And "benchmarks/problem2/scenario1/demo/" is restored to empty with only ".gitkeep"
