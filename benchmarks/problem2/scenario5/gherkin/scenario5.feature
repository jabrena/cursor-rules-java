Feature: Scenario 5 — Case 5 direct OpenSpec implementation
  ...
  #   benchmarks/problem2/scenario5/demo/ without reading or invoking /implement-spec.
  ...
  # - Do NOT read benchmarks/problem2/scenario5/specs/functional-requirements/ directly as agent input
  #   (those files exist only so OpenSpec derivation links resolve within the harness).
  # - Under benchmarks/problem2/scenario5/results/, read ONLY README.md and example.result.json
  #   (operator/metrics template).
  # - Do NOT read prior run JSON files under benchmarks/problem2/scenario5/results/.
  # - Do NOT use examples/openspec/ or other scenarios as input authority.

  Background:
    Given a Case 5 benchmark run for scenario "scenario5"
    And the run case id is "case-5-direct-openspec-problem2"
    And results are stored under "benchmarks/problem2/scenario5/results/"
    And the Case 5 allowlist is the only authorized reading set for requirements and product behavior:
      ...
      | benchmarks/problem2/scenario5/gherkin/scenario5.feature |
      | benchmarks/problem2/metrics-v1.schema.json |
      | benchmarks/problem2/metrics-v1.example.json |
    And the Case 5 results allowlist is the only authorized reading set under "benchmarks/problem2/scenario5/results/":
      | path |
      | benchmarks/problem2/scenario5/results/README.md |
      | benchmarks/problem2/scenario5/results/example.result.json |
  ...
  Scenario: Case 5 run records minimal v1 metrics as JSON
    Given the OpenSpec change path "benchmarks/problem2/scenario5/specs/technical-requirements/openspec/changes/add-greek-gods-api"
    ...
    And only files on the Case 5 results allowlist are read under "benchmarks/problem2/scenario5/results/"
    ...
    And no file under "benchmarks/problem2/scenario5/results/" outside the Case 5 results allowlist may be read during the run
    And the agent must not read, open, grep, or search under any path outside "benchmarks/problem2/scenario5/" except harness metrics files on the Case 5 allowlist under "benchmarks/problem2/" and Plinth tooling on the Case 5 Plinth tooling allowlist
    And "benchmarks/problem2/scenario5/specs/functional-requirements/" must not be read directly for requirements or technology choices
    ...
    And no "ADR-*" file outside "benchmarks/problem2/scenario5/" may be read for requirements or technology choices
    And no "openspec/changes/" design, tasks, or spec files outside "benchmarks/problem2/scenario5/" may be read for requirements
    When the benchmark harness asks the agent to implement the OpenSpec change directly in "benchmarks/problem2/scenario5/demo/"
    And the run completes
    Then the happy path in "benchmarks/problem2/scenario5/specs/technical-requirements/openspec/changes/add-greek-gods-api/specs/greek-gods-api/spec.md" passes
    And a result JSON file exists under "benchmarks/problem2/scenario5/results/"
    ...
    And the result JSON field "protocol_labels.case_id" equals "case-5-direct-openspec-problem2"
    And the result JSON field "solution_snapshot.demo_root" equals "benchmarks/problem2/scenario5/demo/"
    ...
    And "benchmarks/problem2/scenario5/demo/" is restored to empty with only ".gitkeep"
    And the OpenSpec change path "benchmarks/problem2/scenario5/specs/technical-requirements/openspec/changes/add-greek-gods-api" is restored to its pre-run contents, including "tasks.md"
